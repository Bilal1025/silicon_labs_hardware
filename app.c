...
void app_button_press_cb(uint8_t button, uint8_t duration) {
    char data[15] = {'\0'}; // Define a buffer for printing

    switch (duration) {
        case APP_BUTTON_PRESS_DURATION_SHORT:
            snprintf(data, sizeof(data), "SHORT %d", button);
            if (button == 0) {
                up_pressed();
            } else if (button == 1) {
                down_pressed();
        ...
        case APP_BUTTON_PRESS_DURATION_MEDIUM:
            snprintf(data, sizeof(data), "MEDIUM %d", button);
            if (button == 0) {
                menu_pressed(false);
            } else if (button == 1) {
                menu_pressed(true);
        ...
        case APP_BUTTON_PRESS_DURATION_LONG:
            snprintf(data, sizeof(data), "LONG %d", button);
            break;
        case APP_BUTTON_PRESS_DURATION_VERYLONG:
            snprintf(data, sizeof(data), "VERY LONG %d", button);
            break;
        default:
            break;
    }
